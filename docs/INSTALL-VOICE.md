...
Third-party app; not on Workbrew's allowlist so we install manually to
`~/Applications/` (same pattern as WezTerm).
...
3. **Drag `OpenSuperWhisper.app` to `~/Applications/`** (not `/Applications/` —
   that's Workbrew-locked).
